...
public final class HudRenderer {
  ...
  private void renderCraftingPanelFilled(
      CraftingSystem craftingSystem,
      int hoveredRecipeIndex,
      int selectedRecipeIndex,
      boolean craftButtonHovered) {
    ...
    shapeRenderer.rect(
        craftPanelX, headerY, craftPanelWidth, craftHeaderHeight + craftPanelPadding * 0.35f);
    ...
    for (int i = 0; i < craftingSystem.getRecipes().size(); i++) {
      ...
      Color base =
          canCraft ? new Color(0.12f, 0.18f, 0.14f, 0.95f) : new Color(0.12f, 0.12f, 0.14f, 0.9f);
      if (selected) {
    ...
    shapeRenderer.rect(
        craftPanelX + craftPanelPadding,
        detailY,
        craftPanelWidth - craftPanelPadding * 2f,
        craftDetailHeight);
  ...
  private float iconsTopY() {
    float headerBottom =
        craftPanelY + craftPanelHeight - craftHeaderHeight - craftPanelPadding * 0.35f;
  ...
  private void renderCraftingContents(
    ...
    for (int i = 0; i < craftingSystem.getRecipes().size(); i++) {
      ...
      if (hasIcon(icon)) {
        spriteBatch.draw(
            icon, iconX(i) + 4f, iconY(i) + 4f, craftIconSize - 8f, craftIconSize - 8f);
      }
      boolean canCraft = craftingSystem.canCraft(i);
      if (!canCraft) {
      ...
      if (selectedRecipeIndex == i) {
        ...
        spriteBatch.draw(
            whiteRegion, iconX(i) - 2f, iconY(i) + craftIconSize - 1f, craftIconSize + 4f, 3f);
        spriteBatch.draw(whiteRegion, iconX(i) - 2f, iconY(i) - 2f, 3f, craftIconSize + 4f);
        spriteBatch.draw(
            whiteRegion, iconX(i) + craftIconSize - 1f, iconY(i) - 2f, 3f, craftIconSize + 4f);
        spriteBatch.setColor(Color.WHITE);
      } else if (hoveredRecipeIndex == i) {
        spriteBatch.setColor(1f, 1f, 1f, 0.18f);
        spriteBatch.draw(
            whiteRegion, iconX(i) - 2f, iconY(i) - 2f, craftIconSize + 4f, craftIconSize + 4f);
        spriteBatch.setColor(Color.WHITE);
      }
    }
    ...
    // Detail area
    int detailIndex =
        selectedRecipeIndex >= 0 && selectedRecipeIndex < craftingSystem.getRecipes().size()
            ? selectedRecipeIndex
    ...
    for (var ingredient : recipe.ingredients()) {
      ...
      font.setColor(
          enough ? new Color(0.82f, 0.86f, 0.9f, 0.95f) : new Color(1f, 0.65f, 0.65f, 1f));
    ...
    if (canCraft) {
      buttonColor =
          craftButtonHovered
              ? new Color(0.34f, 0.82f, 0.45f, 1f)
              : new Color(0.25f, 0.72f, 0.38f, 0.95f);
  ...
  private void renderTooltipText() {
    ...
    font.draw(
        ...
        tooltipY + tooltipPaddingY + tooltipTextHeight);
  }

  private void drawCount(
      SpriteBatch batch, String text, float x, float y, float textWidth, float textHeight) {
    float bgPadX = 3f;
    float bgPadY = 1.5f;
    float bgX = x - bgPadX;
    ...
    batch.draw(
        whiteRegion, Math.round(bgX), Math.round(bgY), Math.round(bgWidth), Math.round(bgHeight));
  ...
  private void drawDurabilityBar(
      SpriteBatch batch, float x, float y, float width, float height, float ratio) {
    // Background
    batch.setColor(0f, 0f, 0f, 0.65f);
  ...
  private void renderHealth(Viewport viewport, com.droiddungeon.player.PlayerStats stats) {
    ...
    Color fill =
        ratio > 0.5f ? new Color(0.25f, 0.85f, 0.35f, 1f) : new Color(0.92f, 0.35f, 0.2f, 1f);
  ...
  public void renderDeathOverlay(Viewport viewport, boolean restartHovered) {
    ...
    shapeRenderer.setColor(
        restartHovered ? new Color(0.32f, 0.65f, 0.9f, 1f) : new Color(0.24f, 0.38f, 0.52f, 1f));
  ...
  public CraftingHit hitTestCrafting(
    ...
    boolean insidePanel =
        world.x >= craftPanelX
            && world.x <= craftPanelX + craftPanelWidth
            && world.y >= craftPanelY
            && world.y <= craftPanelY + craftPanelHeight;
    if (!insidePanel) {
      return CraftingHit.none();
    }
    ...
    for (int i = 0; i < recipeCount; i++) {
      ...
      float y = iconY(i);
      if (world.x >= x
          && world.x <= x + craftIconSize
          && world.y >= y
          && world.y <= y + craftIconSize) {
        iconIndex = i;
        break;
      }
    }
    ...
    float btnY = detailButtonY();
    if (world.x >= btnX
        && world.x <= btnX + craftButtonWidth
        && world.y >= btnY
        && world.y <= btnY + craftButtonHeight) {
      onButton = true;
    }
  ...
  private boolean hasIcon(TextureRegion icon) {
    return icon != null
        && icon.getTexture() != null
        && icon.getRegionWidth() > 0
        && icon.getRegionHeight() > 0;
  }
