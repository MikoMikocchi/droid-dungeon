...
public final class MapOverlay {
  ...
  public MapMarker getTracked() {
    return trackedMarkerId < 0
        ? null
        : markers.stream().filter(m -> m.id() == trackedMarkerId).findFirst().orElse(null);
  }
  ...
  public void render(
    ...
    int sy =
        Math.round(mapArea.y * Gdx.graphics.getBackBufferHeight() / uiViewport.getWorldHeight());
    int sw =
        Math.round(mapArea.width * Gdx.graphics.getBackBufferWidth() / uiViewport.getWorldWidth());
    int sh =
        Math.round(
            mapArea.height * Gdx.graphics.getBackBufferHeight() / uiViewport.getWorldHeight());
