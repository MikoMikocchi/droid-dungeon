...
public final class WorldRenderer {
  ...
  private void renderTrackedPointer(
      Viewport viewport, Grid grid, Player player, MapMarker tracked) {
    if (tracked == null) {
      return;
    }
  ...
  private void renderMiningHighlight(
      MiningTarget target, float gridOriginX, float gridOriginY, float tileSize) {
    if (target == null) {
      return;
    }
  ...
  private void renderEnemies(
      List<Enemy> enemies, float gridOriginX, float gridOriginY, float tileSize) {
  ...
  private void renderCompanionDotty(
      float gridOriginX, float gridOriginY, float tileSize, float companionX, float companionY) {
  ...
  private void renderWeaponFan(
    ...
    float swingScale =
        weaponState.swinging() ? MathUtils.lerp(1f, 1.08f, weaponState.swingProgress()) : 1f;
    ...
    // Diagonal overlay stripes that do not rotate with the fan orientation, clipped to the fan
    // wedge.
    ...
    shapeRenderer.line(
        centerX,
        centerY,
        centerX + outerRadius * MathUtils.cos(startAngle),
        centerY + outerRadius * MathUtils.sin(startAngle));
    shapeRenderer.line(
        centerX,
        centerY,
        centerX + outerRadius * MathUtils.cos(startAngle + weaponState.arcRad()),
        centerY + outerRadius * MathUtils.sin(startAngle + weaponState.arcRad()));
    shapeRenderer.end();
    ...
    for (int y = visible.minTileY; y <= visible.maxTileY; y++) {
      for (int x = visible.minTileX; x <= visible.maxTileX; x++) {
        ...
        if (block != null) {
          ...
          float shade = 0.65f + 0.04f * exposedCardinal + 0.03f * exposedDiagonal;
