...
/**
 * Main lighting system renderer. Renders all lights to a light map framebuffer, then composites
 * with the scene.
 *
 * <p>Pipeline: 1. Clear light map to ambient darkness 2. For each light: render shadow-masked light
 * contribution additively 3. Composite light map over scene using multiply blending
 */
public class LightRenderer implements Disposable {
  ...
  private final Color ambientColor =
      new Color(0.82f, 0.80f, 0.78f, 1f); // Overwritten by coordinator
  ...
  /**
   * Create the radial gradient texture for light rendering. White in center, fading to black at
   * edges (for additive blending onto ambient).
   */
  ...
  private void createNoiseTexture() {
    ...
    for (int y = 0; y < size; y++) {
      for (int x = 0; x < size; x++) {
        ...
        float smoothed =
            noise * 0.6f
                + 0.4f * (0.5f + 0.3f * (float) Math.sin(x * 0.2f) * (float) Math.cos(y * 0.15f));
  ...
  private void loadShaders() {
    ...
    if (!compositeShader.isCompiled()) {
      Gdx.app.error(
          "LightRenderer", "Composite shader compilation failed: " + compositeShader.getLog());
  ...
  /**
   * Render the light map. Light map contains brightness values: ambient in shadows, brighter where
   * lights shine.
   *
  ...
  private void renderLightSimple(float x, float y, float radius, float intensity, Color color) {
    ...
    batch.setColor(color.r * strength, color.g * strength, color.b * strength, 1f);
  ...
  /**
   * Composite the light map over the scene. Uses multiply blending: scene color * light map color.
   * White in light map = no change, darker = scene gets darker.
   *
   * <p>The FBO already contains the correctly transformed light map, so we draw it directly to
   * screen coordinates (1:1 pixel mapping).
   */
  public void compositeLightMap() {
    ...
    batch.setColor(Color.WHITE);
    batch.draw(lightMapRegion, 0, 0, screenW, screenH);
